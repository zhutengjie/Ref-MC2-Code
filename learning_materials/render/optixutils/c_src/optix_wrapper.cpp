...
void createPipeline(const OptixDeviceContext context, const std::string& path, const std::string& cuda_path, 
                const std::string& kernel_name, OptixModule* module, OptixPipeline* pipeline, OptixShaderBindingTable& sbt, OptiXState* state)
{
    char log[2048];
...

    {
        ...
        OptixProgramGroupDesc closesthit_prog_group_desc  = {};
        closesthit_prog_group_desc.kind                   = OPTIX_PROGRAM_GROUP_KIND_HITGROUP;
        closesthit_prog_group_desc.hitgroup.moduleCH            = *module;
        closesthit_prog_group_desc.hitgroup.entryFunctionNameCH = "__closesthit__rg";
        sizeof_log = sizeof( log );
        OPTIX_CHECK_LOG( optixProgramGroupCreate(
                    context,
                    &closesthit_prog_group_desc,
                    1,   // num program groups
                    &program_group_options,
                    log,
                    &sizeof_log,
                    &state->closesthit_prog_group
                    ) );
        ...
        OptixProgramGroupDesc closesthit_occlusion_group_desc  = {};
        closesthit_occlusion_group_desc.kind                   = OPTIX_PROGRAM_GROUP_KIND_HITGROUP;
        closesthit_occlusion_group_desc.hitgroup.moduleCH            = *module;
        closesthit_occlusion_group_desc.hitgroup.entryFunctionNameCH = "__closesthit__occlusion";
        sizeof_log = sizeof( log );
        OPTIX_CHECK_LOG( optixProgramGroupCreate(
                    context,
                    &closesthit_occlusion_group_desc,
                    1,   // num program groups
                    &program_group_options,
                    log,
                    &sizeof_log,
                    &state->closesthit_occlusion_group
                    ) );

        OptixProgramGroupDesc miss_occlusion_group_desc  = {};
        miss_occlusion_group_desc.kind                   = OPTIX_PROGRAM_GROUP_KIND_MISS;
        miss_occlusion_group_desc.miss.module            = *module;
        miss_occlusion_group_desc.miss.entryFunctionName = "__miss__occlusion";
        sizeof_log = sizeof( log );
        OPTIX_CHECK_LOG( optixProgramGroupCreate(
                    context,
                    &miss_occlusion_group_desc,
                    1,   // num program groups
                    &program_group_options,
                    log,
                    &sizeof_log,
                    &state->miss_occlusion_group
                    ) );
    ...
    //
    {
        ...
        OptixProgramGroup program_groups[] = { state->raygen_prog_group, state->miss_prog_group, state->miss_occlusion_group, state->closesthit_prog_group, state->closesthit_occlusion_group };
...
void create_sbt(OptiXState* state, CUdeviceptr normals, CUdeviceptr kd, CUdeviceptr ks)
{
    ...
    OPTIX_CHECK( optixSbtRecordPackHeader( state->miss_occlusion_group, &ms_sbt[1] ) );
    CUDA_CHECK( cudaMemcpy(
    ...
    CUDA_CHECK( cudaMalloc( reinterpret_cast<void**>( &hitgroup_record ), hitgroup_record_size * 2 ) );
    HitGroupRecord hit_sbt[2];

    OPTIX_CHECK( optixSbtRecordPackHeader( state->closesthit_prog_group, &hit_sbt[0]) );
    hit_sbt[0].data.normals = (float*)normals;
    hit_sbt[0].data.kd = (float*)kd;
    hit_sbt[0].data.ks = (float*)ks;

    OPTIX_CHECK( optixSbtRecordPackHeader( state->closesthit_occlusion_group, &hit_sbt[1] ) );
    hit_sbt[1].data.normals = (float*)normals;
    hit_sbt[1].data.kd = (float*)kd;
    hit_sbt[1].data.ks = (float*)ks;
    ...
    state->sbtEnvSampling.raygenRecord                = raygen_record;

    state->sbtEnvSampling.missRecordBase              = miss_record;
    state->sbtEnvSampling.missRecordStrideInBytes     = sizeof( MissRecord );
    state->sbtEnvSampling.missRecordCount             = 2;

    state->sbtEnvSampling.hitgroupRecordBase              = hitgroup_record;
    state->sbtEnvSampling.hitgroupRecordStrideInBytes     = sizeof( HitGroupRecord );
    state->sbtEnvSampling.hitgroupRecordCount             = 2;
...
OptiXStateWrapper::OptiXStateWrapper(const std::string& path, const std::string& cuda_path)
{
    ...
    createPipeline(pState->context, path, cuda_path, "envsampling", &pState->moduleEnvSampling, &pState->pipelineEnvSampling, pState->sbtEnvSampling, pState);
