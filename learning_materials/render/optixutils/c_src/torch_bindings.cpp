...
std::tuple<torch::Tensor, torch::Tensor> env_shade_fwd(
    ...
    torch::Tensor face_kd,
    torch::Tensor face_ks,
    int    depth)
{
    //
    // launch OptiX kernel
    ...
    CUdeviceptr normals = (CUdeviceptr)face_normals.data_ptr<float>();
    CUdeviceptr kd = (CUdeviceptr)face_kd.data_ptr<float>();
    CUdeviceptr ks = (CUdeviceptr)face_ks.data_ptr<float>();

    create_sbt(stateWrapper.pState, normals, kd, ks);
