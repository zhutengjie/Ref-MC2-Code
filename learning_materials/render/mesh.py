...
class Mesh:
    def __init__(self, v_pos=None, t_pos_idx=None, v_nrm=None, t_nrm_idx=None, v_tex=None, t_tex_idx=None, 
        v_tng=None, t_tng_idx=None, material=None, base=None, face_normals=None, face_kd=None, face_ks=None):
        self.v_pos = v_pos
        self.v_nrm = v_nrm
        self.v_tex = v_tex
        self.v_tng = v_tng
        ...
        self.face_kd = face_kd
        self.face_ks = face_ks
    ...
    def copy_none(self, other):
        ...
        if self.face_normals is None:
            self.face_normals = other.face_normals
        if self.face_kd is None:
            self.face_kd = other.face_kd
        if self.face_ks is None:
            self.face_ks = other.face_ks
    ...
    def clone(self):
        ...
        if out.face_normals is not None:
            out.face_normals = out.face_normals.clone().detach()
        if out.face_kd is not None:
            out.face_kd = out.face_kd.clone().detach()
        if out.face_ks is not None:
            out.face_ks = out.face_ks.clone().detach()
        return out
...
def auto_normals(imesh):
    ...
    face_normals = torch.where(util.dot(face_normals, face_normals) > 1e-20, face_normals, torch.tensor([0.0, 0.0, 1.0], dtype=torch.float32, device='cuda'))
    face_normals = util.safe_normalize(face_normals)
    ...
    return Mesh(v_nrm=v_nrm, t_nrm_idx=imesh.t_pos_idx, base=imesh, face_normals=face_normals)
...
def get_kdks(imesh):
    ...
    if 'kd_ks' in material:
        all_tex0 = material['kd_ks'].sample(v0)
        all_tex1 = material['kd_ks'].sample(v1)
        all_tex2 = material['kd_ks'].sample(v2)
        all_tex = (all_tex0 + all_tex1 + all_tex2) / 3
    else:
        all_tex0 = material['mlp'].sample(v0)
        all_tex1 = material['mlp'].sample(v1)
        all_tex2 = material['mlp'].sample(v2)
        all_tex = (all_tex0 + all_tex1 + all_tex2) / 3

    assert all_tex.shape[-1] == 6, "Combined kd_ks must be 6 channels"
    kd, ks = all_tex[..., 0:3], all_tex[..., 3:6]

    return Mesh(base=imesh, face_kd=kd, face_ks=ks)
