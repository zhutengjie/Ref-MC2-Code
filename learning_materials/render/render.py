...
def shade(
    ...
    if bsdf == 'pbr' or bsdf == 'diffuse' or bsdf == 'white':
        ...
        global rnd_seed
        face_normals = mesh.face_normals.contiguous().clone().detach()
        face_kd = mesh.face_kd.contiguous().clone().detach()
        face_ks = mesh.face_ks.contiguous().clone().detach()
        ...
        diffuse_accum, specular_accum = ou.optix_env_shade(optix_ctx, rast[..., -1], ro, gb_pos, gb_normal, view_pos, kd, ks, 
                            ...
                            rnd_seed=None if FLAGS.decorrelated else rnd_seed, shadow_scale=shadow_scale, face_normals=face_normals, face_kd=face_kd, face_ks=face_ks,  depth=FLAGS.depth)
        rnd_seed += 1
...
def render_layer(
    ...
    buffers = shade(FLAGS, rast_out_s, gb_depth, gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv,
        view_pos, lgt, mesh.material, optix_ctx, mesh, bsdf, shadow_scale)
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(FLAGS, v_pos_clip, rast, rast_db, mesh, view_pos, lgt, resolution, spp, msaa, optix_ctx, bsdf, shadow_scale), rast, rast_db)]
