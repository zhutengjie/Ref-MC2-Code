...
def xatlas_uvmap(glctx, geometry, mat, FLAGS):
    ...
    new_mesh.material['mlp']  =  new_mesh.material['kd_ks']

    path = os.path.join(FLAGS.out_dir, 'model.pth')
    torch.save(new_mesh.material['kd_ks'].state_dict(), path)

    del new_mesh.material['kd_ks']

    for param in new_mesh.material['mlp'].parameters():
...
def validate_itr(glctx, target, ref_mesh, geometry, opt_material, lgt, FLAGS, iter=0):
    result_dict = {}
    with torch.no_grad():
...
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS):
    ...
    with open(os.path.join(out_dir, 'metrics.txt'), 'w') as fout:
        ...
        for it, target in enumerate(dataloader_validate):
            ...
            result_image, result_dict = validate_itr(glctx, target, dataset_validate.getMesh(), geometry, opt_material, lgt, FLAGS)
    ...
    for it, target in enumerate(dataloader_train):
        ...
        if display_image or save_image:
            result_image, result_dict = validate_itr(glctx_display, prepare_batch(next(v_it), FLAGS.train_res, FLAGS.background),
                dataset_validate.getMesh(), geometry, opt_material, lgt, FLAGS, it)
        ...
        img_loss, reg_loss = geometry.tick(glctx, target, lgt, opt_material, image_loss_fn, it, FLAGS)
...
if __name__ == "__main__":
    ...
    path = os.path.join('geo',FLAGS.ref_mesh.split('/')[-1] + '.obj')
    base_mesh = mesh.load_mesh(path)
    ...
    new_mesh = mesh.Mesh(v_tex=uvs, t_tex_idx=faces, base=base_mesh)
    geometry = DLMesh(new_mesh, FLAGS)
    ...
    geometry, mat = optimize_mesh(glctx, glctx_display, geometry, mat, lgt, dataset_train, dataset_validate, FLAGS, pass_idx=0, pass_name="dmtet_pass1", 
        optimize_light=FLAGS.learn_lighting, optimize_geometry=False)
    pass_idx = 1
    ...
    geometry, mat = optimize_mesh(glctx, glctx_display, geometry, mat, lgt, dataset_train, dataset_validate, FLAGS, 
                pass_idx=pass_idx, pass_name="mesh_pass", warmup_iter=100, 
                optimize_light= True, optimize_geometry=False ) # Use warmup to avoid nasty Adam spikes
    ...
    FLAGS.samples = 32
    if FLAGS.validate:
        validate(glctx_display, geometry, mat, lgt, dataset_validate, os.path.join(FLAGS.out_dir, "validate"), FLAGS)
