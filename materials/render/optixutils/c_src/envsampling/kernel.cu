    ...
    float3 gb_kd, float3 gb_ks, float pdfSum, float weight, float3 &diff, float3 &spec, float3 diff_grad, float3 spec_grad)
{
    ...
    RadiancePRD prd;
    prd.V = 0;
    prd.shaded_color = make_float3(0.0f);
    prd.depth = 1;
    unsigned int u0;
    unsigned int u1;
    ...
    {
        if(prd.V==1)
        V = 1 * params.shadow_scale + 1 * (1 - params.shadow_scale);
        if(prd.V==0)
        V = 0 * params.shadow_scale + 0 * (1 - params.shadow_scale);
    }
    else
    {
        if(prd.V==1)
    ...
    if(prd.V==0 && (params.depth>1))
    {
        light_col = prd.shaded_color;
    }
    ...
    if (params.backward) 
    {
        ...
        if (params.depth==1)
        {
          _diff_grad = diff_grad * light_col * mis_weight * weight * V;
          _spec_grad = spec_grad * light_col * mis_weight * weight * V;
        }

        else{
            _diff_grad = diff_grad * light_col * mis_weight * weight;
            _spec_grad = spec_grad * light_col * mis_weight * weight;
        }
    ...
    if(params.depth == 1)
    {
      diff = _diff * light_col * mis_weight * weight * V;
      spec = _spec * light_col * mis_weight * weight * V;
    }

    else
    {
      diff = _diff * light_col * mis_weight * weight;
      spec = _spec * light_col * mis_weight * weight;
    }
}

__device__ void process_sample_occlusion(float3 ray_origin, float3 ray_dir, float3 gb_pos, float3 gb_normal, float3 gb_view_pos, 
    float3 gb_kd, float3 gb_ks, float pdfSum, float weight, float3 &diff, float3 &spec, int& shadow, int depth)
{
...
extern "C" __global__ void __closesthit__rg()
{
    ...
    int depth = prd->depth + 1;
    int n = 0;
    if (depth==2)
    {
      n = params.n_samples_x;
    }
    else
    {
      n = params.n_samples_x/2;
    }
    const int    prim_idx        = optixGetPrimitiveIndex();
    const float3 ray_dir         = optixGetWorldRayDirection();
    
    float3 gb_normal = make_float3(rt_data->normals[3*prim_idx], rt_data->normals[3*prim_idx + 1], rt_data->normals[3*prim_idx + 2]);
    float3 gb_kd = make_float3(rt_data->kd[3*prim_idx], rt_data->kd[3*prim_idx + 1], rt_data->kd[3*prim_idx + 2]);
    float3 gb_ks = make_float3(rt_data->ks[3*prim_idx], rt_data->ks[3*prim_idx + 1], rt_data->ks[3*prim_idx + 2]);
    float3 view_pos = optixGetWorldRayOrigin();
    const float3 P    = optixGetWorldRayOrigin() + optixGetRayTmax()*ray_dir; // 交点
    float3 wo = safe_normalize(optixGetWorldRayOrigin() - P );

    float d = dot(gb_normal, wo);
    if(d<0)
    {
        gb_normal = -1 * gb_normal;
    }

    float strata_frac = 1.0f / n;
    float sample_frac = 1.0f / (n * n);
    float alpha = gb_ks.y * gb_ks.y; // roughness squared
    ...
    unsigned int rng_state = hash_pcg(params.rnd_seed, prim_idx * 7);
    unsigned int lightIdx = rand_pcg(rng_state) % params.perms.size(0), bsdfIdx = rand_pcg(rng_state) % params.perms.size(0);
    ...
    for (int i = 0; i < n * n; ++i)
    {
        ...
        // Light importance sampling
        sx = ((float)(params.perms[lightIdx][i] % n) + uniform_pcg(rng_state)) * strata_frac;
        sy = ((float)(params.perms[lightIdx][i] / n) + uniform_pcg(rng_state)) * strata_frac;
        ray_dir = lightSample(sx, sy, pdf_light);
        pdf_bsdf = bsdf_pdf(pDiffuse, pSpecular, gb_normal, wo, ray_dir, alpha);
        process_sample_occlusion(P, ray_dir, P, gb_normal, view_pos, gb_kd, gb_ks, pdf_light + pdf_bsdf, sample_frac, diff, spec, shadow, depth);
        diffAccum = diffAccum + diff;
        specAccum = specAccum + spec;

        // BSDF sampling (sample either the diffuse or specular lobe)
        sx = ((float)(params.perms[bsdfIdx][i] % n) + uniform_pcg(rng_state)) * strata_frac;
        sy = ((float)(params.perms[bsdfIdx][i] / n) + uniform_pcg(rng_state)) * strata_frac;
        sz = uniform_pcg(rng_state);
        ray_dir = bsdf_sample(pDiffuse, pSpecular, gb_normal, wo, make_float3(sx, sy, sz), alpha, pdf_bsdf);
        pdf_light = lightPDF(ray_dir);
        process_sample_occlusion(P, ray_dir, P, gb_normal, view_pos, gb_kd, gb_ks, pdf_light + pdf_bsdf, sample_frac, diff, spec, shadow, depth);
    ...
    float3 shaded_color = diffAccum * (1 - gb_ks.z) * gb_kd +  specAccum;
    float weight;
    if(shadow != 0)
    {weight = (n * n * 2 - shadow) / shadow;}

    prd->shaded_color = shaded_color + weight * shaded_color * 0.2f;

    if(prd->shaded_color.x<0.0f)
    prd->shaded_color.x=0.0f;
    if(prd->shaded_color.x>1.0f)
    prd->shaded_color.x=1.0f;
    if(prd->shaded_color.y<0.0f)
    prd->shaded_color.y=0.0f;
    if(prd->shaded_color.y>1.0f)
    prd->shaded_color.y=1.0f;
    if(prd->shaded_color.z<0.0f)
    prd->shaded_color.z=0.0f;
    if(prd->shaded_color.z>1.0f)
    prd->shaded_color.z=1.0f;
